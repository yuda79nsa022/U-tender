...
import { approveOffer, submitReview, addDrawings } from "./actions";
...
export default async function ProjectDetailPage({ params }: { params: { id: string } }) {
  ...
  // Signed URLs so drawings stay behind the private bucket even when linked.
  // Expiry is tied to the bid deadline — contractors need access for the
  // full bidding window, not just an hour.
  ...
  const { data: offers } = await supabase
    .from("offers")
    .select("*, contractor_profiles(company_name, avg_rating, review_count)")
    .eq("project_id", project.id)
    .order("amount", { ascending: true });

  const deadlinePassed = new Date(project.bid_deadline) < new Date();

  const approvedOffer = offers?.find((o: any) => o.status === "approved");
  const { data: existingReview } = approvedOffer
    ? await supabase.from("reviews").select("*").eq("project_id", project.id).maybeSingle()
    : { data: null };
        ...
        <div>
          ...
          {drawingLinks.length > 0 && (
            <a
              ...
              className="mt-2.5 inline-block font-mono text-xs text-blue underline"
            >
              Download all as .zip ({drawingLinks.length} file{drawingLinks.length === 1 ? "" : "s"})
            </a>
          )}
          ...
          <form
            action={async (formData) => {
              "use server";
              await addDrawings(project.id, formData);
            }}
                  ...
                  <tr key={o.id} className="border-b border-border">
                    <td className="py-3 px-2.5">
                      <div className="font-display font-semibold text-[13.5px]">
                        {o.contractor_profiles?.company_name ?? "Contractor"}
                      </div>
                      {o.message && <div className="text-xs text-steel-light mt-0.5 max-w-xs">{o.message}</div>}
                    </td>
                    <td className="py-3 px-2.5">
                      <span className="text-amber text-[11px] tracking-tight">
                        {stars(o.contractor_profiles?.avg_rating ?? 0)}
                      </span>{" "}
                      <span className="font-mono text-[11px] text-steel">
                        ({o.contractor_profiles?.review_count ?? 0})
                      </span>
                    </td>
                    ...
                    <td className="py-3 px-2.5">
                      {project.status === "awarded" ? (
                        ...
                        <form
                          action={async () => {
                            "use server";
                            await approveOffer(project.id, o.id);
                          }}
                        >
                          <button
                            type="submit"
                            className="bg-navy hover:bg-navy-deep text-white text-xs font-semibold rounded px-3 py-1.5"
                          >
                            Approve
                          </button>
                        </form>
      ...
      {approvedOffer && (
        <div className="mt-8 max-w-xl">
          <h3 className="font-mono text-[11px] uppercase tracking-wide text-navy mb-2">
            Rate {approvedOffer.contractor_profiles?.company_name ?? "the contractor"}
          </h3>
          {existingReview ? (
            ...
            <form action={submitReview} className="bg-white border border-border rounded px-4.5 py-4 grid gap-3.5">
              <input type="hidden" name="project_id" value={project.id} />
              <input type="hidden" name="contractor_id" value={approvedOffer.contractor_id} />
              <RatingInput />
              <textarea
                name="comment"
                rows={3}
