...
export default function NewProjectPage({ searchParams }: { searchParams: { error?: string } }) {
  // A sensible default deadline of 7 days out, so the datetime field
  // isn't blank and owners don't accidentally post a same-day deadline.
  const defaultDeadline = new Date(Date.now() + 7 * 24 * 60 * 60 * 1000);
  const defaultDeadlineValue = defaultDeadline.toISOString().slice(0, 16);
  ...
  return (
    <main className="max-w-4xl mx-auto px-5 py-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1.4fr_1fr] gap-6 items-start">
        <form action={createProject} className="grid gap-[18px]">
          <div>
          ...
          <div>
            ...
            <input
              name="trade"
              placeholder="e.g. Roofing, Framing, Fencing"
              className="w-full border border-border rounded px-3 py-2.5 text-sm"
            />
          </div>
          <div>
          ...
          <div>
            ...
            <div className="border border-dashed border-blue bg-blue-tint rounded px-4 py-6 text-center">
              ...
              />
              <p className="text-[11px] text-blue mt-2">
                PDF, DWG, JPG, PNG, or a .zip folder of drawings — up to 50MB total
              </p>
            </div>
            <p className="text-xs text-steel-light mt-1.5">
              Only approved, subscribed contractors can view these files.
            </p>
          </div>
          <div>
            <label className="block font-mono text-[11px] uppercase tracking-wide text-steel mb-1.5">
            ...
            />
            <p className="text-xs text-steel-light mt-1.5">
              No offers are accepted after this time.
            </p>
          </div>
          <button
            type="submit"
            className="bg-amber hover:bg-amber-dark text-white font-semibold text-sm rounded px-5 py-2.5 w-fit mt-1"
          >
            Post project
          </button>
        </form>
